...
void CPage::GetStatusCode(string headerBuf)
{
	...
	char *pcTmp = new char[strlen(headerBuf.c_str())];
	char *charIndex = NULL;

	strncpy(pcTmp, headerBuf.c_str(), strlen(headerBuf.c_str()));
	charIndex = strstr(pcTmp, "http/");
...
void CPage::GetContentLength(string headerBuf)
{
	...
	char *pcTmp = new char[strlen(headerBuf.c_str())];
	strncpy(pcTmp, headerBuf.c_str(), strlen(headerBuf.c_str()));
	char *charIndex = strstr(pcTmp, "content-length");
